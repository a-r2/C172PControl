...
ACT_LEN = 8 #[deltaa, deltae, deltaf, deltar, deltat, deltam]
...
class Actuation():
    def __init__(self, act_type, rx2act_out, act2csv_in, act2tx_in, event_start):
        ...
        self.proc.start()
    def random_control(self, rx2act_out, act2csv_in, act2tx_in, event_start):
        ...
        while True:
            ...
            if (np.any(rxdata[:,0] >= self.t)):
                ...
                self.actstr += '\n'
                act2tx_in.send(self.actstr) #send actuation to transmit TX telemetry
                act2csv_in.send(self.csvact) #send actuation to store in CSV
            else:
                pass
